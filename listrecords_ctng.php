<div class="col-lg-6 col-md-6 form-group ">CT/NG Record All </div>
<div class="col-lg-6 col-md-6 form-group text-right">
    <button class=" btn btn-sm btn-success" onclick="location.replace('index.php?task=create_ctng&pid=<?=$_GET['pid']?>')"><i class="fa fa-file"></i> Create</button>
    <button class=" btn btn-sm btn-info"><i class="fa fa-database"></i> Excel</button>
</div>
<hr>
<div>
    <table class="table table-bordered table-striped mb-none" id="datatable-ctng">
        <thead>
            <tr>
                <th>Visit Code</th>
                <th>Date Coll</th>
        ...
        <tbody>
            <?
    $get_task = 'ctng';
    $sql_eia = "select * from ctng where ptid = '".$_GET['pid']."' order by visit_code desc";
