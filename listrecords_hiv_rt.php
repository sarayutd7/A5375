<div class="col-lg-6 col-md-6 form-group ">HIV Rapid Test Record All </div>
<div class="col-lg-6 col-md-6 form-group text-right">
    <button class=" btn btn-sm btn-success" onclick="location.replace('index.php?task=create_hiv_rt&pid=<?=$_GET['pid']?>')"><i class="fa fa-file"></i> Create</button>
...
<div>
    <table class="table table-bordered table-striped mb-none" id="datatable-details">
        ...
        <tbody>
            <?
    $get_task = 'hiv_rt';
    $sql_eia = "select * from hiv where ptid = '".$_GET['pid']."' order by visit_code desc";
    ...
    for($i=0;$i<$nr_eia;$i++){ ?>
            <tr>
                ...
                <td class="center">
                    ...
                    <? } ?>
                    <? if($result_eia[$i]['rapid']!=''){ ?> 
                    <a href="index.php?task=print_<?=$get_task?>&pid=<?=$result_eia[$i]['ptid']?>&recid=<?=$result_eia[$i]['id_record']?>" class="btn btn-xs btn-info" title="Print Record"><i class="fa fa-print"></i> Rihes</a>
                    <? } ?>
                    <? /*if($result_eia[$i]['rapid']!=''){ ?> 
                    <a href="index.php?task=print_<?=$get_task?>&pid=<?=$result_eia[$i]['ptid']?>&recid=<?=$result_eia[$i]['id_record']?>&labpiman=1" class="btn btn-xs btn-primary" title="Print @Piman Record"><i class="fa fa-print"></i> Piman</a>
                    <? }*/ ?>
