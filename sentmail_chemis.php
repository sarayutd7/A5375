...
?>
   <section class="panel">
    <header class="panel-heading">
        <h2 class="panel-title">Form Send Mail Lab Chemistries.
        </h2>
    </header>
    <div class="panel-body">
      <form action="http://dmu1.rihes.cmu.ac.th/sendmail/A5375.php?mail=chem&rd=<?=$result_current_data['id_record']?>&u=<?=$result_userLogin['user']?>&uid=<?=$result_userLogin['id']?>" method="POST" enctype="multipart/form-data">
        ...
        <div class="row form-group">
         ...
         <div class="row form-group">
            ...
            <div class="col-lg-11 col-sm-11 col-xs-12">
                ...
                <div class="col-lg-11 col-sm-11 col-xs-12">
                    <input type="text" class="form-control" id="titlemail" name="titlename" value="Chemistries Report Lab Alert <?=webdetail('protocal')?> [ PTID - <?=$result_current_data['ptid']?>] - [Visit - <?=$result_current_data['visit_code']?>] ">
                </div>
            </div>
        </div>
        ...
        <div class="row text-center">
            <? if($_GET['task'] == 'sentmail_chemis'){ ?>
            <button class="btn btn-primary" type="submit"> <i class="fa fa-paper-plane"></i> Send</button>
            <? } ?>
            <button class="btn btn-danger" type="reset" onclick="location.replace('index.php?pid=<?=$result_current_data['ptid']?>&task=findPID&tab=Chemis');"> <i class="fa fa-ban"></i> Cancel</button>
        </div>
        </form>
    </div>
</section>
<?  include('_footer_repeat_datetime.php'); include('_js_chemis.php');  ?>
<? if($_GET['task'] == 'viewmail_chemis'){ ?>
<script>
