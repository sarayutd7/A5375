...
function extra_nameLab($labText,$exText,$result){
    switch($labText){
            
        /*case 'WBC' :   */ 
        case 'Platelets' : $nresult = $result*1000;
            $txt = $labText." &nbsp;(".number_format($nresult,',')." /mm<sup>3</sup>)"; 
            break;
        case 'Absolute Neutrophil' : 
            $nresult = $result*1000;
            $txt = $labText." ".$exText." <br>&nbsp;(".number_format($nresult,',')." cells/mm<sup>3</sup>)"; break; 
    ...
    <tbody>
       <tr>
        ...
        <tr>
            ...
            <td class="text-left">&nbsp;
                <?=extra_nameLab(ucfirst($result_getlistLab[$row]['test']),"",current_data(strtolower(str_replace(' ','_',$result_getlistLab[$row]['test'])),$_GET['recid']))?>
            </td>
        ...
        <tr>
            ...
            <td class="text-left">&nbsp;
                <? //ucfirst($result_getlistLab[$row]['test'])?> 
                <?=extra_nameLab(ucfirst($result_getlistLab[$row]['test']),"Count",current_data(strtolower(str_replace(' ','_',$result_getlistLab[$row]['test'])),$_GET['recid']));?>
            </td>
            <td align="center"><?=current_data(strtolower(str_replace(' ','_',$result_getlistLab[$row]['test'])),$_GET['recid'])?></td>
            <td align="center">
                <?=$result_getlistLab[$row]['units']?> </td>
